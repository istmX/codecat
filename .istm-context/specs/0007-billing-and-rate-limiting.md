...
## Requirements
- **Plan Tiers**:
  - *Free Plan*: Limit of X files per PR, limit of Y reviews per hour.
- **Rate Limiting**:
  - Track review timestamps per user/repository in the database.
  - If a user triggers a review and exceeds their rate limit, abort and return a specific error indicating the cooldown time remaining.
- **File Limit**:
  - When fetching a PR diff, truncate or reject if it exceeds the maximum allowed files/tokens for the user's plan.
- **UI & Motion**:
  ...
  - **Token Usage UI**: Display how many files/tokens were used on the `ReviewResults` page.

## Data Model
- **User Model**: Add `planTier` (default: "FREE").
- **Review Model**: Add `tokensUsed` (Int) and `filesProcessed` (Int).

## Build Plan
1. Update Prisma schema to track `planTier` on `User` and `tokensUsed`/`filesProcessed` on `Review`.
